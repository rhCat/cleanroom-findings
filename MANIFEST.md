...
| [curl-etag-save-write-discard](findings/curl-etag-save-write-discard/) | Bug cleanroom | CONFIRMED | `src/tool_cb_hdr.c` — `save_etag()` checks `ftruncate` but not `fwrite`/`fputc`/`fflush`; `--etag-save` write failure leaves an empty etag file while curl exits 0 |
| [curl-etag-save-write-discard-patch](findings/curl-etag-save-write-discard-patch/) | Patch cleanroom | VERIFIED | write-result check added; etag write failure now exits 23 (`CURL_WRITEFUNC_ERROR`), happy path unchanged |
